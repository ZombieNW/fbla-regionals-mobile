<script>
	...
	const gifDuration = 1000; // IMPORTANT: Adjust this (in ms) to match your GIF's actual length
...
{#if isVisible}
	<div
		...
		style="left: {position.x - 192}px; top: {position.y - 64}px;"
		transition:fade={{ duration: 50 }}
		bind:this={selfElement}
	>
		<div class="relative">
			<!-- Bind src to reactive variable to force reload -->
			<img src={currentGifSrc} alt="Attack effect" class="h-32 w-[32rem] opacity-0" />
