...
var jsCurry = (function () {
	...
	if (!Function.prototype.bind)
		Function.prototype.bind = function (thisArg /* , ... */) {
			var f = this,
				args0 = sliceMF.call(arguments, 1);
			return function (/* ... */) {
	...
	function F(x /* , ... */) {
		// F() shorthand notation for some basic operations
		if (typeof x == 'function') return F.curry.apply(undefined, arguments);
		if (arguments.length == 2) {
			var y = arguments[1];
			if (typeof x == 'string') return y[x].bind(y);
			if (typeof y == 'function') return (typeof x == 'number' ? F.aritize : F.partial)(x, y);
		}
	...
	F.partial = function (a, f) {
		// 'a' supplies some arguments to 'f'
		var n = a.length;
		return function (/* ... */) {
	...
	F.o = function (/* ... */) {
		// composition of 1 or more functions
		var fs = arguments;
		return function (/* ... */) {
			var n = fs.length,
				res = fs[--n].apply(undefined, arguments);
			while (n > 0) res = fs[--n](res);
			return res;
		};
	};
	F.oMap = function (f, g) {
		// composition, using F.map(g, <arguments>)
		return function (/* ... */) {
			return F.applyF(f, F.map(g, arguments));
		};
	};
	F.aritize = function (n, f) {
		// for discarding optional trailing arguments
		return function (/* ... */) {
	...
	F.cmpLex = function (cmpE, v, w) {
		// "lexicographic order"; cmpE need not return a number
		return (
			F.any(function (e, i) {
	...
	F.concatArgs = F.oMap(F('concat', []), function (a) {
...
var jqMath = (function () {
	...
	function M(x, y, z) /* any number of arguments */ {
		...
		if (x.nodeType == 1 /* Element */ && x.tagName.toLowerCase() == 'math') return M.eToMathE(x);
		F.err(err_M_);
	}
	...
	M.setAttrs = function (e, attrsP) /* uses attr.name/value/[specified for IE6-7] */ {
		...
		F.iter(function (attr) {
			if (attr.specified !== false /* for IE6-7 */) e.setAttribute(attr.name, attr.value);
		}, attrsP || []);
		return e;
	};
	...
	M.addClass = function (e, ws) {
		...
		if (typeof e.className != 'undefined') {
			// needed for old IE, works for non-XML
			var classes = e.className;
			e.className = (classes ? classes + ' ' : '') + ws;
		} else {
			// needed for XML, would work for non-IE
	...
	M.hasClass = function (e, w) {
		// works for HTML or XML elements, unlike jQuery e.g. 1.4.3
	...
	function appendMeArgs(
		e,
		args /* null/undefined, string, node, or array-like of nodes incl.
			live NodeList */,
		attrsP
	) {
		if (args == null) {
		} else if (typeof args == 'string') e.appendChild(e.ownerDocument.createTextNode(args));
	...
	(function () {
		// M.webkitVersion, M.operaVersion, M.msieVersion, M.mozillaVersion
		var ua = navigator.userAgent.toLowerCase(),
			match = ua.match(/webkit[ \/](\d+)\.(\d+)/);
		if (match) {
			M.webkitVersion = [Number(match[1]), Number(match[2])];
			fixMathMLQ_ = M.webkitVersion[0] <= 540 /*@@*/;
		} else {
			match =
				ua.match(/(opera)(?:.*version)?[ \/]([\w.]+)/) ||
				ua.match(/(msie) ([\w.]+)/) ||
	...
	function checkMathMLAttrs(e) {
		...
		if (tagName == 'mi') {
			if (!e.getAttribute('mathvariant') && e.firstChild && e.firstChild.nodeType == 3 /* Text */)
				e.setAttribute('mathvariant', e.firstChild.data.length == 1 ? 'italic' : 'normal');
		} else if (tagName == 'mo') {
			...
			//@@ combine with M.newMe 'mspace' clause
	...
	(function () {
		if (self.location) {
			...
			if (match) M.MathML = !match[1];
				...
				(M.webkitVersion && F.cmpLex(F.cmpX, M.webkitVersion, [537, 17]) < 0) ||
				M.operaVersion
			)
				M.MathML = false;
		}
	...
	var mtokens_ = ['mn', 'mi', 'mo', 'mtext', 'mspace', 'ms'],
		impMRows_ = ['fmath', 'msqrt', 'mtd', 'mstyle', 'merror', 'mpadded', 'mphantom', 'menclose'],
		accentDsByS_ = {
			// top and bottom space in ems
			'\xAF' /* ¯ macron */: [0, 0.85],
			'\u203E' /* ‾ overline */: [0, 0.85],
			'\u02D9' /* ˙ dot above */: [0, 0.75],
			'\u02C7' /* ˇ caron */: [0, 0.7],
	...
	M.newMe = function (tagName, argsP /* for appendMeArgs() */, attrsP, docP) {
		...
		if (
			...
			var c = a.length == 1 ? a[0] : {};
			if (c.nodeType == 3 /* Text */ && /[\]|([{‖)}]/.test(c.data)) M.addClass(e$, 'fm-mo-Luc');
		} else if (tagName == 'mspace') {
			...
			var up = F.applyF(Math.max, ups),
				dn = F.applyF(Math.max, dns);
			if (up > 0.65 || dn > 0.65) {
				e$.fmUp = up;
				e$.fmDn = dn;
			...
			var num$ = docP.createElement('td'),
				den$ = docP.createElement('td');
			...
			var tr1$ = docP.createElement('tr'),
				tr2$ = docP.createElement('tr'),
				tbody$ = docP.createElement('tbody'),
				table$ = docP.createElement('table'),
			...
			var t = 0.06 * (ups[0] + dns[0]),
				up = ups[0] + t + 0.1,
				dn = dns[0];
			if (tagName == 'mroot') {
				var ht = 0.6 * (ups[1] + dns[1]),
					d = 0.25 / 0.6 - 0.25;
			...
			for (var i = 1; i < a.length; i++) {
				...
				//	css('verticalAlign', d.toFixed(2)+'em');
				if (M.msieVersion && (document.documentMode || M.msieVersion) < 8) a[i].style.zoom = 1; // to set hasLayout
				if (tagName == 'mmultiscripts') (preAP || postA).push(a[i].parentNode);
			}
			if (tagName == 'mmultiscripts') {
				e$ = document.createElement('mrow');
				var ap = (preAP || []).concat(a[0], postA);
				for (var i = 0; i < ap.length; i++) e$.appendChild(ap[i]);
			...
			for (var i = 0; i < a.length; i++) e$.appendChild(a[i]);
			e$.className = 'fm-mtr';
			...
			// return <td> partly so rowspan/colspan work
			...
			var e = M.setAttrs(e$, attrsP);
			attrsP = undefined;
			var rowspan = e.getAttribute('rowspan'),
				colspan = e.getAttribute('columnspan');
			if (rowspan) {
	...
	M.mfencedToMRowArgs = function (e) {
		...
		var res = [
				newMo(F.defOr(M.getSpecAttrP(e, 'open'), '(')),
	...
	M.spaceMe = function (
		widthS,
		docP /* incl. avoid bad font support for \u2009 &thinsp; */
	...
	M.fenceMe = function (me1, leftP, rightP, docP) {
		return M.newMe(
			'mrow',
			[M.newMe('mo', F.defOr(leftP, '('), docP), me1, M.newMe('mo', F.defOr(rightP, ')'), docP)],
			docP
		);
	};
	F.iter(
	...
	M.eToMathE = function (mathE) /* 'mathE' has been parsed from MathML syntax */ {
		...
		if (M.MathML && mathE.tagName == 'math') return fixMathMLQ_ ? fixMathMLDeep(mathE) : mathE;
		var doc = mathE.ownerDocument;
		function newMeDeep(me) {
			var tagName = me.tagName.toLowerCase(),
				args = me.childNodes;
	...
	M.decimalComma = function (qP) /* whether to allow decimal commas */ {
		if (qP != null) {
			...
			M.re_ /* .lastIndex set during use */ = RegExp(
				'(' + numPat + ')|' + escPat_ + '|' + M.surrPair_ + '|\\S' + M.combiningChar_ + '*',
				'g'
			);
		}
		return decComma_;
	...
	var commaLangs =
		'af|an|ar|av|az|ba|be|bg|bs|ca|ce|co|cs|cu|cv|da|de|el|es|et|eu|fi|fo|fr|' +
	...
	M.infix_ = {
		// operator precedences, see http://www.w3.org/TR/MathML3/appendixc.html
		'⊂⃒': 240,
		'⊃⃒': 240,
		'≪̸': 260,
		'≫̸': 260,
	...
	function spScan() {
		...
		return [M.spaceMe(widthS, docP_), /^[^-]*[1-9]/.test(widthS) ? '\u2009' /* &thinsp; */ : null];
	}
	function braceScan() {
		var tokP = scanTokP();
		if (tokP && tokP[1] == '↖' && !tokP[0]) {
			...
			if (!(meTokP_tokP[0] && tokP && tokP[1] == '}' && !tokP[0]))
				throw (
					'Expected an embellished operator and "}" after "{↖", before position ' + M.re_.lastIndex
				);
	...
	function attrScan(nameP, mmlOnlyQ) {
		// note usually doesn't affect non-MathML rendering
		if (!nameP) nameP = scanWord('attribute name');
		var v = scanString(nameP + ' attribute'),
			tok = scanMeTok(nameP);
		if (!mmlOnlyQ || M.MathML) tok[0].setAttribute(nameP, v);
		return tok;
	}
	function clScan() {
		// note currently ignored by MathPlayer
		var desc = 'CSS class name(s)',
			ws = scanString(desc),
			tok = scanMeTok(desc);
		M.addClass(tok[0], ws);
		return tok;
	...
	function mvScan(sP) {
		var s = sP || scanString('mathvariant'),
			tok = scanMeTok(s),
			me = tok[0];
		if (!F.elem(M.mtagName(me), ['mi', 'mn', 'mo', 'mtext', 'mspace', 'ms']))
			throw (
				'Can only apply a mathvariant to a MathML token (atomic) element, at ' +
				'position ' +
				M.re_.lastIndex
			);
	...
	function meScan(tagNameP) {
		...
		return [newMe_(tagNameP, tok[0]), F.elem(tagNameP, ['mstyle', 'mpadded']) ? tok[1] : null];
	}
	function phantomScan(macroS) {
		var tok = scanMeTok(macroS);
		F.iter(function (e) {
			e.disabled = true;
		}, tok[0].createElement('input'));
		if (macroS == 'vphantom')
			tok[0] = newMe_('mpadded', tok[0], { width: '0', style: 'display: inline-block; width: 0' });
	...
	function minsizeScan() {
		var s = scanString('minsize'),
			tok = scanMeTok('minsize'),
			me = tok[0];
		if (M.mtagName(me) != 'mo')
	...
	function binomScan() {
		...
		var xMe = scanMeTok('\\binom')[0],
			yMe = scanMeTok('\\binom')[0],
	...
	M.macros_ /* each returns a tokP */ = {
		...
		mi: F(mtokenScan, 'mi'),
		mo: F(mtokenScan, 'mo'),
		text: F(mtokenScan, 'mtext'),
		html: htmlScan,
		sp: spScan,
		attr: attrScan,
		attrMML: F(attrScan, null, true),
		id: F(attrScan, 'id'),
		dir: F(attrScan, 'dir'),
		cl: clScan,
		mv: mvScan,
		bo: F(mvScan, 'bold'),
		it: F(mvScan, 'italic'),
		bi: F(mvScan, 'bold-italic'),
		sc: F(mvScan, 'script'),
		bs: F(mvScan, 'bold-script'),
		fr: F(mvScan, 'fraktur'),
		ds: F(mvScan, 'double-struck'),
		bf: F(mvScan, 'bold-fraktur'),
		mstyle: F(meScan, 'mstyle'),
		merror: F(meScan, 'merror'),
		mpadded: F(meScan, 'mpadded'),
		phantom: F(phantomScan, 'phantom'),
		vphantom: F(phantomScan, 'vphantom'),
	...
	function scanTokP() {
		...
		var s1 = match[2] || match[0],
			mvP = null;
		if (/^[_^}\u2196\u2199]$/.test(match[0]) || (match[2] && M.macro1s_[s1])) return [null, s1];
		...
		var opSP = M.infix_[s1] || M.prefix_[s1] || M.postfix_[s1] ? s1 : null,
			e;
		if (s1 == '\u2009' /* &thinsp; */)
			// incl. avoid bad font support, incl. in MathML
	...
	function parse_mtd_tokP(optQ /* => mtd can be null or an mtr */) {
		var mxP_tokP = parse_mxP_tokP(M.infix_[',']),
			tokP = mxP_tokP[1] || scanTokP(),
			mxP = mxP_tokP[0];
		if (!mxP) {
	...
	function parse_rowspan_tokP() {
		var v = scanString('rowspan'),
			mtd_tokP = parse_mtd_tokP(),
			mtd = mtd_tokP[0];
	...
	function parse_mtr_tokP(optQ /* => mtr can be null */) {
		var mtds = [];
		while (true) {
			var mtdP_tokP = parse_mtd_tokP(mtds.length == 0),
				mtdP = mtdP_tokP[0],
			...
			if (!(tokP && tokP[1] == ','))
				return [
					mtds.length || !optQ || (tokP && tokP[1] == ';') ? M.newMe('mtr', mtds, docP_) : null,
					tokP
				];
		}
	}
	...
	function parse_table_tokP(dtableQ) {
		...
		var mtrs = [];
		while (true) {
			var mtrP_tokP = parse_mtr_tokP(mtrs.length == 0),
				mtrP = mtrP_tokP[0],
				tokP = mtrP_tokP[1] || scanTokP();
			if (mtrP) mtrs.push(mtrP);
			if (!(tokP && tokP[1] == ';'))
				return [newMe_('mtable', mtrs, dtableQ ? { displaystyle: true } : undefined), tokP];
		}
	}
	...
	M.macro1s_ /* each returns mxP_tokP, so can do precedence-based look-ahead */ = {
		mtd: parse_mtd_tokP,
		rowspan: parse_rowspan_tokP,
		colspan: parse_colspan_tokP,
		mtr: parse_mtr_tokP,
		dtable: F(parse_table_tokP, true),
		ttable: F(parse_table_tokP, false),
		table: F(parse_table_tokP, undefined),
			...
			if (k == 'uo' || !tokP || (tokP[0] ? meTokP : embelKP(tokP[1]) != 'ss')) {
				...
				if (!meTokP) {
					if (!tokP || !tokP[0]) meTokP = [emptyMe_(), null];
						...
						tokP = scanTokP();
						var postA = [];
						while (tokP && !tokP[0] && embelKP(tokP[1]) == 'ss') postA.push(parseMxs());
				...
				F.iter(function (mxsP) {
					if (!mxsP) a.push(M.newMe('mprescripts', undefined, docP_));
						...
						a.push(
							mxsP['_'] || M.newMe('none', undefined, docP_),
	...
	function parse_mxP_tokP(prec, tokP) /* tokP may be non-atomic */ {
		...
		while (true) {
			...
			if (!op || (mx0p && (tokP[0] ? !(M.infix_[op] || M.postfix_[op]) : M.macro1s_[op]))) {
				if (!mx0p) {
					mx0p = tokP[0];
					tokP = null;
				} else {
					if (prec >= precAdj_) break;
					var mxP_tokP = parse_mxP_tokP(precAdj_, tokP),
						mx1 = mxP_tokP[0];
				...
				if (moP) {
					...
					var meTok_tokP = parseEmbel(tokP),
						a = [];
					...
					if (a.length == 1) mx0p = a[0];
						...
						if (op == '\u2009' /* &thinsp; */ || (precL || precROpt) >= precAdj_) {
							...
							if (op == '=') k = 'infix-loose';
								...
								// a.length == 3 && not fences
								k = op == ',' || op == ';' ? 'separator' : precL <= 270 ? 'infix-loose' : 'infix';
							...
							if (!M.MathML && k && !moP.style.fontSize) M.addClass(extOp, 'fm-' + k);
	...
	if (
		...
		(document.readyState && document.readyState !== 'loading' && !document.documentElement.doScroll)
	) {
		dom_loaded();
